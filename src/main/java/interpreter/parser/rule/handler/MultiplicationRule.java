...
public class MultiplicationRule extends ExpressionRule {

    @Override
    public ExpressionRule next() {
        return new DivisionRule();
    }
    ...
    public ExpressionNode handle(List<Token> statement) {
        ...
        for (Token t : statement) {
            if (t.tokenType().equals(TokenType.ASTERISK)) {
        ...
        throw new ParserException("inconsistent multiplication at line " + t.line() + ", between column " +
                t.fromColumn() + " and " + t.toColumn() + ".");
    }

    @Override
    public boolean matches(List<Token> statement) {
        return contains(statement, TokenType.ASTERISK);
    }
}
